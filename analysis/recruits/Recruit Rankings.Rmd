...
df <- read.csv("/Users/samivanecky/Downloads/D2 Women's Recruit Rankings - ALL.csv")
```
...
# 5k XC
r5k <- df %>%
  filter(!is.na(X5k..XC.) & is.finite(X5k..XC.)) %>%
  arrange(desc(X5k..XC.)) %>%
  mutate(
    Rank_5k_XC = 1:n()
  ) %>%
...
# Write Data to CSV
```{r}
write.csv(teamRank, "teamRankD2Women.csv")
write.csv(rankDf, "runnerRankD2Women.csv")
```

# Visuals
## Weighted Team Score
```{r}
ggplot(teamRank, aes(reorder(Team, Weighted_Score), Weighted_Score, fill = Team)) +
  geom_bar(stat = "identity") +
  ggtitle("Teams by Weighted Score", subtitle = "Using top 5 recruits per school.") +
  theme_minimal() + 
  labs(
    x = "Team",
    y = "Weighted Score"
...
## Weighted Score V2
```{r}
ggplot(teamRank, aes(reorder(Team, Weighted_Score_V2), Weighted_Score_V2, fill = Team)) +
  geom_bar(stat = "identity") +
  ggtitle("Teams by Weighted Score (V2)", subtitle = "Weighted score V2: 75% avg. weighted score, 25% best weighted score.") +
  theme_minimal() + 
  labs(
    x = "Team",
    y = "Weighted Score"
