...
grpYrly <- rl %>%
  ...
  summarise(
    pr = min(MARK_TIME, na.rm = T),
    prs_run = sum(is_pr, na.rm = T),
    avg_time = round(mean(MARK_TIME, na.rm = T), 2),
...
# Line item level
raceGrp <- rl %>%
  filter(IS_FIELD == FALSE & MARK_TIME != -999) %>%
  mutate(
    TIME = MARK_TIME
  ) %>%
...
raceDets %>%
  ...
  filter(
   DIVISION == "D1" & GENDER == "F" & competition_year == "2021" & EVENT %in% c("Mile", "800m", "3000m", "5000m")
  ) %>%
  ungroup() %>%
