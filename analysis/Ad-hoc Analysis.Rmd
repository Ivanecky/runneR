...
# Join SOC data
soc <- raceSOC %>%
  # inner_join(raceSOCTopTime, by = c("MEET_NAME", "MEET_DATE", "competition_year", "GENDER", "EVENT", "EVENT_TYPE")) %>%
  left_join(cal, by = c("MEET_DATE" = "cal_d")) %>%
...
soc <- soc %>%
  mutate(
    soc = round(((0.1 * (month_best_time / winning_time)) + ((c_month_best_time / avg_top_time) * 0.4) + ((c_month_best_time / winning_time) * 0.5)), 5) * 100
  ) %>%
...
# Create a race performance metric
raceDets <- raceDets %>%
  ...
  ungroup() %>%
  mutate(
    wsor = ((soc / 100) * weighted_finish) * 2 + (1 - weighted_finish_time) * (soc / 100) * 3 + (weighted_overall_time) * 5 # + (weighted_time) * 1.5
  ) %>%
  filter(
    !(EVENT == "Mile" & EVENT_TYPE != "INDOOR") &
    !(EVENT == "10K" & TIME < 1000) &
...
soc %>%
  ...
  geom_bar(stat = "identity") +
  geom_label(aes(label = soc), fill = "white", size = 8) +
  ggplot2::labs(
    x = "",
    y = "SOC",
    caption = "Omitting XC competitions.",
    title = "Top 15 Women's Races Based on Strength of Competition (SOC)",
    subtitle = "Races shown from 2017-2022 seasons."
  ) +
  ggplot2::theme_bw() +
  scale_fill_stata() +
  ggplot2::theme(
    aspect.ratio = 9 / 16,
    axis.text.y = element_text(face = "italic"),
    plot.title = ggplot2::element_text(size = 30, hjust = 0.5, vjust = 0.5, face = "bold"),
    plot.subtitle = element_text(face = "italic"),
...
raceDets %>%
  ...
  filter(
   DIVISION == "D1" & GENDER == "F" & competition_year == "2021"
  ) %>%
  ungroup() %>%
  slice_max(n = 25, order_by = wsor) %>%
  ggplot(aes(reorder(paste0(NAME, " - ", TEAM, " - ", MEET_NAME), wsor), wsor, fill = as.factor(EVENT))) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = wsor), fill = "white", size = 7) +
  ggplot2::labs(
    x = "",
    y = "WSOR",
    caption = "Omitting XC competitions.",
    title = "Top 25 D1 Women's Races of 2022 Season",
...
raceDets %>%
  ...
  slice_max(n = 25, order_by = wsor) %>%
  select(
    NAME, TEAM, EVENT, PRELIM, clean_time, MEET_NAME, wsor
  ) %>%
  rename(
    Name = NAME,
    Team = TEAM,
...
raceDets %>%
  ...
  filter(
   DIVISION == "D1" & GENDER == "F" & competition_year == 2021 & EVENT_TYPE == "INDOOR"
  ) %>%
  group_by(NAME, TEAM) %>%
  summarise(
  ...
  ungroup() %>%
  filter(n_races >= 2) %>%
  slice_max(n = 50, order_by = avg_wsor) %>%
  ggplot(aes(reorder(NAME, avg_wsor), avg_wsor, fill = as.factor(TEAM))) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = avg_wsor), fill = "white", size = 7) +
  ggplot2::labs(
    x = "",
    y = "Avg. WSOR",
    caption = "",
    title = "Top 50 D1 Women of 2021 Indoor Season",
    subtitle = "Races are based on WSOR (Weighted Score of Run)."
  ) +
  ggplot2::theme_bw() +
  ggplot2::theme(
