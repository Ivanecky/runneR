...
tab <- raceDets %>%
  ...
  filter(
   DIVISION == div & GENDER == "M" & competition_year == 2022 & EVENT %in% c("Mile", "800m", "3000m", "5000m", "1500m", "10k", "3000S")
  ) %>%
  ungroup() %>%
  ...
  gtExtras::gt_theme_538() %>%
  gt::tab_header(title = paste(glue::glue("Top {rows} Performances by NCAA {div} Men in 2022 Based on RPI"))) %>%
