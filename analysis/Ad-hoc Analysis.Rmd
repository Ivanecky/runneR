...
# Line item level
raceGrp <- rl %>%
  filter(IS_FIELD == FALSE & converted_time != -999) %>%
  mutate(
    TIME = converted_time
  ) %>%
...
raceDets <- raceDets %>%
  ...
  ungroup() %>%
  mutate(
    wsor = ((soc / 100) * weighted_finish) * 2 + (1 - weighted_finish_time) * (soc / 100) * 1.5 + (weighted_overall_time) * 6 # + (weighted_time) * 1.5
  ) %>%
  filter(
    !(EVENT == "Mile" & EVENT_TYPE != "INDOOR") &
    !(EVENT == "10K" & TIME < 1000) &
...
soc %>%
  ungroup() %>%
  funique() %>%
  filter(GENDER == "F" & competition_year > 2018) %>%
  filter(EVENT %in% c("800m", "Mile", "1500m", "3000m", "5000m", "10k", "3000S")) %>%
  slice_max(n = 15, order_by = soc) %>%
  ggplot(aes(reorder(paste0(MEET_NAME, "-", EVENT, " (", competition_year, ")"), soc), soc, fill = as.factor(competition_year))) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = soc), fill = "white", size = 4) +
  ggplot2::labs(
    x = "",
    y = "SOC",
...
raceDets %>%
  ...
  filter(
   DIVISION == "D1" & GENDER == "F" & competition_year == "2022" & EVENT %in% c("Mile", "800m", "3000m", "5000m", "1500m", "10k")
  ) %>%
  ungroup() %>%
  slice_max(n = 25, order_by = wsor) %>%
  ggplot(aes(reorder(paste0(NAME, " - ", TEAM, " - ", MEET_NAME, " (", EVENT, ")"), wsor), wsor, fill = as.factor(EVENT))) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = wsor), fill = "white", size = 2) +
  ggplot2::labs(
    x = "",
    y = "WSOR",
    caption = "Omitting XC competitions.",
...
raceDets %>%
  ...
  slice_max(n = 25, order_by = wsor) %>%
  select(
    NAME, TEAM, EVENT, PRELIM, MARK, MEET_NAME, wsor
  ) %>%
  rename(
    Name = NAME,
    Team = TEAM,
