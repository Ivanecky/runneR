...
raceDets %>%
  ...
  filter(
   DIVISION == "D1" & GENDER == "F" & competition_year == "2022" & EVENT %in% c("Mile", "800m", "3000m", "5000m", "1500m", "10k", "3000S") & 
     EVENT_TYPE == "OUTDOOR"
  ) %>%
  ungroup() %>%
...
tab <- raceDets %>%
  ...
  filter(
   DIVISION == div & GENDER == "F" & competition_year >= 2019 & EVENT %in% c("Mile", "800m", "3000m", "5000m", "1500m", "10k", "3000S")
  ) %>%
  ungroup() %>%
  ...
  gt::tab_header(title = paste(glue::glue("Top {rows} Performances by NCAA {div} Women Since 2019 Based on RPI"))) %>%
